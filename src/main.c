...
int main(int argc, char *argv[]) {
    ...
    time_t dob1 = create_timestamp(1975, 2, 26, 0, 0, 1, 0);
    strcpy(current->id,"0000000001\0");
    strcpy(current->name,"Simo Vilen\0");
    strcpy(current->email,"pikkutimo@gmail.com\0");
    current->date_of_birth = dob1;

    printf("PERSON %-15s%-32s%-32s%-30s\n", current->id, current->name, current->email, ctime(&current->date_of_birth));
